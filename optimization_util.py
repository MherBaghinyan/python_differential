...
def nonlinear_optimality(image_matrixes, x_b_image_matrix, k_value, vector_len, t_value):
    ...
    for i in range(len(x_b_array)):
        cons.append({'type': 'ineq', 'fun': lambda x: np.array([x_b_array[i].evalf(subs={t: x[0]})])})
