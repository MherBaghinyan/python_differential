...
def z_nonlinear_optimality(image_matrixes, x_b_image_matrix, k_value, vector_len, t_value):
    ...
    for i in range(len(f_array)):
        if not is_number(f_array[i]):
            f_cons.append({'type': 'ineq', 'fun': lambda z: np.array([f_array[i].evalf(subs={t: z[0]})])})

    if len(f_cons) > 0:
        f_res = minimize(func_z, 0.0, args=(-1.0,), constraints=f_cons, method='SLSQP')
        print(f_res.x[0])
        return f_res.x[0]
