...
def exponential_c_2(image, k_value, t_value):
    ...
    for k in range(0, k_value + 1):
        item += (1/factorial(k_value - k)) * e_image_2(image, k, t_value)
    return item
...
def recover_e_image_values(image, k_value, t_value):
    item = 0
    for k in range(0, k_value + 1):
        exp_value = exponential_c_2(image, k, t_value)
        item += ((t-t_value) ** k) * exp_value
        print("K = " + str(k))
        print("C_ (" + str(k) + ") = " + str(exp_value))
        print("X (" + str(k) + ") = " + str(e_image_2(image, k, t_value)))
        with open("Output.txt", "a") as text_file:
            print("K = " + str(k) + " C_ (" + str(k) + ") = " + str(exp_value) + " X (" + str(k) + ") = " + str(e_image_2(image, k, t_value)), file=text_file)
...
def cooperative_matrix_1(matrix_a, matrix_b, k, t_value_):
...
# cooperative_matrix_1(matrix_a, matrix_b, k, t_value_)
