...
from project_files.cooperative.cooperative_dirichle import *
from project_files.services.graph_gui import *


def graph_cd_window(root, matrix, t_value):
    graph_cd_root = Toplevel(root)
    graph_cd_root.title("cooperative game report")
    graph_cd_root.geometry("1000x400")

    xf = Frame(graph_cd_root, relief=GROOVE, borderwidth=2)
    Label(graph_cd_root, text='parametric values of cooperation matrix ').place(relx=0.1, rely=0.10, anchor=NW)
    ...
    def create_graph_window(f_index):
        graph_window(graph_cd_root, f_vec[f_index], 0, 1.1)
